...
int main(int, char**) {
	...
	struct layer *l2 = CREATE_HIDDEN_LAYER(&network, dataset.hidden_nodes, 0, SIGMOID);

	printf("antwerp: establishing network connections\n");

	ret = initialise_layers(&network, l1, l2, NULL);
	if(ret == -1) return -1;

	struct layer *input = create_input_layer(&network, l1, dataset.input_nodes);
	if(input == NULL) return -1;

	struct layer *output = create_output_layer(&network, l2, SIGMOID, dataset.output_nodes);
